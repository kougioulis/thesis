...
where \(\sigma(x) = (1 + e^{-x})^{-1}\) is the sigmoid transformation and each element \(\hat{\mathbb{A}}_{ij\tau}\) represents the confidence of the model that a directed edge \(X^i_{t-\tau} \rightarrow X^j_t\) exists. The final sigmoid activation converts logits into interpretable probabilities of causal edge existence.
