...
\newcommand{\BriefDescription}{$\bullet$
    {\em Óýíôïìç ÐåñéãñáöÞ:} \\}

\newcommand{\CheckedConstraint}
   {$\bullet$ {\em Åëåã÷ôáßïé Äïìéêïß Ðåñéïñéóìïß :} \\}

\newcommand{\ExecutionAnalysis}[1]{
    $\bullet$ {\em ÓõíÝðåéåò ÅêôÝëåóçò :} \\
    \parbox[t]{2cm}{\hspace{2cm}} \ \parbox[t]{12cm}{ {\small #1} }
}

\newcommand{\SemanticAnalysis}[1]{
    $\bullet$ {\em ÐñïûðïèÝóåéò ÓçìáóéïëïãéêÞò Ïñèüôçôáò :} \\
    \parbox[t]{2cm}{\hspace{2cm}} \ \parbox[t]{12cm}{ {\small #1} }
}

\newcommand{\namecheck}{ ¸ëåã÷ïò óõíÝðåéáò óôçí ïíïìáôïäïóßá }

%--------  ---------------- --------------

\newcommand{\EFupdate}[3]{
    \item  {\scriptsize Ôýðïò}: #1 {\scriptsize Ïñßóìáôá} : $#2$

    \hspace*{1cm} \parbox[t]{12cm}{#3}
}

\newcommand{\visiblecomment}[1]{
  \underline{\centerline{ \bf Ó÷üëéï}}\\
     \vspace{0.3cm}
     \bc
      \parbox[t]{14cm}{
            \small \em #1
    ...
    \ec
  \underline{\centerline{ \bf ÔÝëïò Ó÷ïëßïõ}}
}
...
\newcommand{\putTwoFigures}[6]{
    ...
    \begin{figure}
        \begin{minipage}[b]{7cm}
            \centerline{\putsfigure{#1}{#3}}
            \vspace*{3mm} \centerline{(á)}
        \end{minipage}
        \begin{minipage}[b]{7cm}
            \centerline{\putsfigure{#2}{#3}}
            \vspace*{3mm} \centerline{(â)}
        \end{minipage}
        \caption{#5}{\vspace*{0.1cm}
            \centerline{ \parbox[t]{13cm}{\footnotesize #6}}}
        \label{#4}
